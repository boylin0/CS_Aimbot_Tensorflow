      ...
      for index, value in enumerate(classes[0]):
        if scores[0, index] > 0.5:
          if value == 1:
            ...
            targetfile = os.path.join('collected_image','img_'+str(imagenum)+'.jpg')
            while os.path.isfile(targetfile):
              imagenum+=1
              targetfile = os.path.join('collected_image','img_'+str(imagenum)+'.jpg')
            cv2.imwrite( targetfile , image_np, [int(cv2.IMWRITE_JPEG_QUALITY), 90])
            print('Saving... ({})'.format('img_'+str(imagenum)+'.jpg'))
            time.sleep(0.5)
